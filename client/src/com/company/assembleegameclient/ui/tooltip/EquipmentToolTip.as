...
private function makeLegendaryExtraText():void {
    ...
    private function addTierText():void
    {
        ...
        if ((!this.objectXML.hasOwnProperty("Consumable")) || (!this.objectXML.hasOwnProperty("Treasure"))) ;
        {
            if (this.objectXML.hasOwnProperty("Tier")) {
                tier = XML(this.objectXML.Tier);
                this.tierText.setStringBuilder(new StaticStringBuilder((!isNaN(Number(this.objectXML.Tier)) ? "T" + this.objectXML.Tier : this.objectXML.Tier)));
                this.tierText.setColor(tier.hasOwnProperty("@color") && tier.@color <= 0xFFFFFF ? tier.@color : 0xFFFFFF);
                this.tierText.setBold(tier.hasOwnProperty("@bold") && tier.@bold == "true");
                this.tierText.setHTML(true); //so can use <br> for newline html
            }
            else
            {
    ...
    private function addRoFToEffectsList():void {
        if (((this.objectXML.hasOwnProperty("RateOfFire")) && (!(this.comparisonResults.processedTags[this.objectXML.RateOfFire[0].toXMLString()]) == true))) {
            if (!this.comparisonResults.processedTags[this.objectXML.RateOfFire[0].toXMLString()]) {
    ...
    private function addCdToEffectsList():void {
        if (((this.objectXML.hasOwnProperty("Cooldown")) && (!(this.comparisonResults.processedTags[this.objectXML.Cooldown[0].toXMLString()]) == true))) {
            if (!this.comparisonResults.processedTags[this.objectXML.Cooldown[0].toXMLString()]) {
    ...
    private function makeRestrictionList():void {
        ...
        //Specialty
        if (this.objectXML.hasOwnProperty("@legendary")) {
            this.restrictions.push(new Restriction("Specialty: Legendary", 0xFFFF00, true));
        }
        if (this.objectXML.hasOwnProperty("@epic")) {
            this.restrictions.push(new Restriction("Specialty: Epic", 0x572cb2, false));
        }
        if (this.objectXML.hasOwnProperty("@setType")) {
            this.restrictions.push(new Restriction("Specialty: Artifact", 0xFF9900, false));
        }
        if (this.objectXML.hasOwnProperty("@unique")) {
            this.restrictions.push(new Restriction("Specialty: Unique", 0xFFce00, false));
        }
        if (this.objectXML.hasOwnProperty("@unholy")) { //unholy tier
            this.restrictions.push(new Restriction("Specialty: Unholy", 0x6F28CC, false));
        }
        if (this.objectXML.hasOwnProperty("@unholylg")) { //unholy legendary tier
            this.restrictions.push(new Restriction("Specialty: (Unholy) Legendary", 0x6F28CC, true));
        }
        //RoF, Cooldown, DropRate
        //if (this.objectXML.hasOwnProperty("RateOfFire")) {
        //    this.restrictions.push(new Restriction("Rate of Fire: " + Math.round(this.objectXML.RateOfFire * 100) + "%", 0xFFFFFF, true));
        //}
        //if (this.objectXML.hasOwnProperty("Cooldown")) {
        //    this.restrictions.push(new Restriction("Cooldown: " + this.objectXML.Cooldown + " seconds", 0xFFFFFF, true));
        //}
        //if (this.objectXML.hasOwnProperty("@droprate")) { //rather use this one
        //    this.restrictions.push(new Restriction("Drop Rate: " + this.objectXML.@droprate + "%", 0x00ff00, false));
        //}
        //
        //if (this.objectXML.hasOwnProperty("Droprate")) { //to be unused soon, won't remove though
        //    this.restrictions.push(new Restriction("Drop Rate: " + this.objectXML.Droprate + "%", 0x00ff00, false));
        //}
        //Rarity
        if (this.objectXML.hasOwnProperty("Common")) {
            this.restrictions.push(new Restriction("Rarity: Common", 0x72ff59, false));
        }
        if (this.objectXML.hasOwnProperty("Uncommon")) {
            this.restrictions.push(new Restriction("Rarity: Uncommon", 0x72ff59, false));
        }
        if (this.objectXML.hasOwnProperty("Rare")) {
            this.restrictions.push(new Restriction("Rarity: Rare", 0x249eff, false));
        }
